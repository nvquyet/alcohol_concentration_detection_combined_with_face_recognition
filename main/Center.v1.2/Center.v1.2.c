...
bool BLOCK = 1, buttonFlash = 0, isMatchFaceID = 0;
...
void setup()
{
  ...
  pinMode(MQ3Pin,INPUT_PULLUP); //D0 Nong độ cồn
...
//      }
      ...
      if (millis() - previousMillis > 1000)   // có thể dùng time của blynk
      {
        ...
        if(isMatchFaceID == 1)
        {
          //Serial.println("isMatchFaceID = 1");
          isMatchFaceID = 0;
        }
        else
        {
          //Serial.println("isMatchFaceID = 0");
        }
      }
